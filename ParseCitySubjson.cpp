...
void ParseCitySubjson(vector<City>& cities, const Json& json, Country_data country_data) {
    for (const auto& city_json : json.AsList()) {
        const auto& city_obj = city_json.AsObject();
        cities.push_back({ city_obj["name"s].AsString(), city_obj["iso_code"s].AsString(),
                          country_phone_code + city_obj["phone_code"s].AsString(), country_data.country_name, country_data.country_iso_code,
                          country_data.country_time_zone, country_data.languages });
    }
}
